...
class VESC(object):
    def __init__(self, serial_port, has_sensor=False, start_heartbeat=False, baudrate=115200, timeout=0.05, get_timeout = 1.0):
        """
        ...
        self.serial_port = serial.Serial(port=serial_port, baudrate=baudrate, timeout=timeout, exclusive = True)
        if has_sensor:
        ...
        if (self.wheels):        
            ...
            if num_read_bytes is not None:
                ...
                while self.serial_port.in_waiting <= num_read_bytes:
                    if (time.time() - t0 > self.get_timeout):
                        ok = False
                        break
                if ok:
            ...
            if num_read_bytes is not None:
                ...
                while self.serial_port.in_waiting <= num_read_bytes + 5:
                    time.sleep(0.016)  # add some delay just to help the CPU

                #print(num_read_bytes, self.serial_port.in_waiting)
                val = self.serial_port.read(self.serial_port.in_waiting)
                response, consumed = decode(val)
